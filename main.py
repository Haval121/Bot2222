...
DB_PATH = "bot_data.db"
GITHUB_DB_URL = "https://raw.githubusercontent.com/Haval121/Boytttt/main/bot_data.db"
...
# ----------------- Database Sync from GitHub -----------------
async def sync_db_from_github(app):
    try:
        print("🔄 داگرتنی داتابەیس لە GitHub...")
        urllib.request.urlretrieve(GITHUB_DB_URL, DB_PATH)
        print("✅ داتابەیس بە سەرکەوتوویی لە گیتهەبەوە داگیرا.")
    except Exception as e:
        print(f"⚠️ کێشە لە داگرتنی داتابەیس لە گیتهەب: {e}")
...
def main():
    ...
    # Sync Database from GitHub on Startup
    asyncio.get_event_loop().run_until_complete(sync_db_from_github(app))
    init_db()
